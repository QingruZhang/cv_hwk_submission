...
def get_model_and_optimizer(args) -> Tuple[nn.Module, torch.optim.Optimizer]:
    ...
    if args.arch == "SimpleSegmentationNet":
        ...
        parameter_group = [
            ...
            {"params": model.resnet.parameters(), "lr": args.base_lr},
        ]
    elif args.arch == "PSPNet":
        model = PSPNet(args.layers, bins=[1,2,3,6], num_classes=args.classes, 
                zoom_factor=args.zoom_factor, use_ppm=args.use_ppm, pretrained=args.pretrained,) 
        parameter_group = [
            # {"params": model.ppm.parameters()+model.cls.parameters()+model.aux.parameters(), "lr":args.base_lr*10}, 
        ...
        raise ValueError("Invalid arch argument %s."%args.arch)
...
def get_train_transform(args) -> transform.Compose:
    ...
    train_transform = transform.Compose(
            [
                ...
                transform.RandRotate([args.rotate_min, args.rotate_max], mean, args.ignore_label),
